...
class SetupADBNode(BaseNode):
    def node_setup_adb(self, input={}):
        print("node_setup_adb",input["adb_path"], input["adb_port"], input["retry_count"])
        res = ADBClass.AdbSingleton.getInstance().connectDevice(adb_path=input["adb_path"], adb_port=input["adb_port"], retryCount=input["retry_count"])
        input["adb_is_connected"] = res
        return input

    def __init__(self, parentFlow,nodeName, parentNode, nextNode, isRootNode):
        inputFunc = self.node_setup_adb
        super().__init__(parentFlow,nodeName, parentNode, nextNode, inputFunc, isRootNode)

class StartAppNode(BaseNode):
    def node_startapp_adb(self, input={}):

        print("node_startapp_adb",input["adb_path"], input["adb_port"])
        ...
        if ADBClass.AdbSingleton.APP_PACKAGE in res:
            ...
            input["adb_is_started"] = True
        return input

    def __init__(self,parentFlow, nodeName, parentNode, nextNode, isRootNode):
        inputFunc = self.node_startapp_adb
        super().__init__(parentFlow,nodeName, parentNode, nextNode, inputFunc, isRootNode)

class ScreenShotNode(BaseNode):
    def node_screenshot_adb(self, input={}):
        print("node_screenshot_adb",input["adb_path"], input["adb_port"])
        ADBClass.AdbSingleton.getInstance().screen_capture(self.screenshotName)
        # ADBClass.AdbSingleton.getInstance().screen_capture("loginCapture.png")
        input["cv2_img_screenshot"] = self.screenshotName


        return input

    def __init__(self,parentFlow, nodeName, parentNode, nextNode, isRootNode, screenshotName):
        inputFunc = self.node_screenshot_adb
        super().__init__(parentFlow,nodeName, parentNode, nextNode, inputFunc, isRootNode)
        self.screenshotName = screenshotName

class ClickNode(BaseNode):
    def node_click_adb(self, input={}):
        print("node_screenshot_adb",input["adb_path"], input["adb_port"])
        ADBClass.AdbSingleton.getInstance().tap(input["tap_pos"])
        # ADBClass.AdbSingleton.getInstance().screen_capture("loginCapture.png")

        return input

    def __init__(self,parentFlow, nodeName, parentNode, nextNode, isRootNode):
        inputFunc = self.node_click_adb
        super().__init__(parentFlow,nodeName, parentNode, nextNode, inputFunc, isRootNode)

class LoginRewardNode(BaseNode):
    def cv2CheckImgExist(self, patternPath, screenshotPath):
        ...
        if len(locations[0]) > 0:
            ...
            return None
    def login_reward_nod_adb(self, input={}):
        time.sleep(10)
        print("node_screenshot_adb",input["adb_path"], input["adb_port"])
        # ADBClass.AdbSingleton.getInstance().tap(input["tap_pos"])
        # ADBClass.AdbSingleton.getInstance().screen_capture("loginCapture.png")
        ...
        while isLoading is True:
            ...
            isSpeedAnimation = OctoUtil.OctoUtil.check_pixel_color('./img/loginReward.png', 751, 35,
                                                                   (45, 49, 60, 255))
    ...
    def __init__(self,parentFlow, nodeName, parentNode, nextNode, isRootNode):
        inputFunc = self.login_reward_nod_adb
        super().__init__(parentFlow,nodeName, parentNode, nextNode, inputFunc, isRootNode)

class cvMatchScreenshotNode(RetryNode):
    def node_screenshot_adb(self, input={}):
        print("node_screenshot_adb",input["adb_path"], input["adb_port"])
        ADBClass.AdbSingleton.getInstance().screen_capture(self.screenshotName)
        print("SCREENSHOT")
        ...
        # self.node_cv_match_template(input)
        print("node_cv_match_template", self.pattern, input["cv2_img_screenshot"])
        screenshot = cv2.imread(self.screenshotName, 0)
        ...
        if len(locations[0]) > 0:
            ...
            x, y = locations[::-1]
            w, h = pattern.shape[::-1]
            input["cv2_img_screenshot_rect"] = (x[0], y[0], w, h)
            input["tap_pos"] = (x[0] + w/2, y[0] + h/2)
        else:
            ...
            if self.subPattern is not None:
                for subPattern in self.subPattern:
                    ...
                    if len(subPatternLocations[0]) > 0:
                        ...
                        print("function: ", subPattern["function"], "parameters", subPattern["parameters"])
                        subPattern["function"](subPattern["parameters"])
                        input["subPatternRes"] = True
                        break
                    else:
                        input["subPatternRes"] = False
    ...
    def verify_cv_result(self, verifyInput):
        return len( verifyInput["cv2_img_screenshot_result"][0]) > 0 or verifyInput["subPatternRes"] is True
        # extnd the chcking range to include subpattern result chk

    def __init__(self,parentFlow, nodeName, parentNode, nextNode, isRootNode, screenshotName, iconName, retryCount = 10, retryInterval: float = 1, subPattern = None):
        inputFunc = self.node_screenshot_adb
        ...
        self.pattern = iconName

class tapUntilMatchFixedPos(RetryNode):
    def node_click_adb(self, input={}):
        print("node_screenshot_adb", input["adb_path"], input["adb_port"])
        ADBClass.AdbSingleton.getInstance().tap(self.tapPos)
        # ADBClass.AdbSingleton.getInstance().screen_capture("loginCapture.png")

        return input


    def log_in_check(self, input):
        print("node_screenshot_adb", input["adb_path"], input["adb_port"])
        ADBClass.AdbSingleton.getInstance().screen_capture(self.screenshotName)
        print("SCREENSHOT")
        # ADBClass.AdbSingleton.getInstance().screen_capture("loginCapture.png")
        ...
        return len(input["cv2_img_screenshot_result"][0]) > 0

    def __init__(self,parentFlow, nodeName, parentNode, nextNode, isRootNode, screenshotName, iconName,tapPos, retryCount = 50, retryInterval: float = 0.5):
        inputFunc = self.node_click_adb
        super().__init__(parentFlow,nodeName, parentNode, nextNode, inputFunc, isRootNode, self.log_in_check ,True, retryCount, retryInterval)
        self.screenshotName = screenshotName
        self.pattern = iconName
        self.tapPos = tapPos
...
class DailyMaterialNode(BaseNode):
    ...
    def GotoMiddleStep(self, destinationPage):
        ...
        if destinationPage[3] == "multi":
            ...
            res = OCRClass.OCRSingleton.getInstance().findTextPosition('./img/GotoMiddleStepScreenshotCroppedScreenshot.png', str(middleNo))
            print("res: ", res)
            if res is not None:
                ADBClass.AdbSingleton.getInstance().tap((res[1][0]+left, res[1][1]+top))
                return True
            else:
                cvres = self.cv2CheckImgExist('./Icons/MiddleLevelENCIdentifier.png', './img/GotoMiddleStepScreenshotCroppedScreenshot.png', False)
    ...
    def startMissionFight(self):
        ...
        for i in range(10):
            ...
            scanRes = OctoUtil.OctoUtil.crop_image('./img/startMission.png', (left, top, right, bottom), './img/startMissionCroppedScreenshot.png')
